...
void SceneDiffuse::setLightParams(QuatCamera camera)
{
	...
	vec3 worldLight = vec3(7.0f,7.0f,7.0f);

	//SET LIGHT VALUES
	prog.setUniform("light.La", 1.0f, 1.0f, 1.0f); //ambient light
	prog.setUniform("light.Ld", 0.9f, 0.9f, 0.9f); //diffuse light
	prog.setUniform("light.Ls", 0.7f, 0.7f, 0.7f); //specular light

	prog.setUniform("light.constant", 1.0f);
	prog.setUniform("light.linear", 0.01f);
	prog.setUniform("light.quad", 0.001f);
...
void SceneDiffuse::render(QuatCamera camera)
{
	...
	prog.setUniform("mat.Ka", 0.1f, 0.1f, 0.1f);
	prog.setUniform("mat.Kd", 0.51f, 1.0f, 0.49f);
	prog.setUniform("mat.Ks", 0.1f, 0.1f, 0.1f);
	plane->render();// what does it do?
	 ...
	 prog.setUniform("mat.Kd", 0.46f, 0.29f, 0.0f); 
	 prog.setUniform("mat.Ks", 0.3f, 0.3f, 0.3f);
	 prog.setUniform("mat.shininess", 32.0f);
